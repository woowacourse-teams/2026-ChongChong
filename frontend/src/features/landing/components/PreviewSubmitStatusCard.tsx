...
import type { AssignmentSubmitStatus } from '../../assignment/types';
...
export default function PreviewSubmitStatusCard({ status }: Props) {
  const titleId = useId();
  const progress = status.memberCount === 0 ? 0 : (status.completeCount / status.memberCount) * 100;

  return (
    <section css={cardStyle} aria-labelledby={titleId}>
      <header css={headerStyle}>
        <h2 id={titleId} css={statusTitleStyle}>
          제출 현황
        </h2>
      </header>
